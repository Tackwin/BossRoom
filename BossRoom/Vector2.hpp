...
struct Vector2 {
	...
	bool isInRec(const Vector2& pos, const Vector2& size) const;
	...
	inline static bool equal(const Vector2 &a, const Vector2 &b, float eps = 0.1f) {
		if(eps == 0)
			return a == b;
		return b.distTo2(a) <= eps * eps;
	}

	void render(sf::RenderTarget& target, Vector2 p = Vector2::ZERO, sf::Color color = sf::Color::White);
};
